...
int main()
{
    ...
    for (int c = 0; c < channel; c++)
    {
        ...
        strcat_s(name, c < 1 ? "_RED" : c < 2 ? "_GREEN" : "_BLUE");
        fopen_s(&codebook, name, "r");
        for (i = 0; i < 1 << new_byte; i++)
            fscanf_s(codebook, "%d", table + i);
        fclose(codebook);

        // Decoding
        int size = total_bit / AREA, bit = 0, y = 0, p_dequant = 0, p_quant = 0;       // Bit window & temp value
        for (i = 0; i < total_bit; i++)
        {
            ...
            y += (x & bit ? 1 : 0) << --new_byte;  // Cumulating value using code
            bit >>= 1;         // Move bit
            if (new_byte == 0)  // Reconstruct
            {
                dequant[c * AREA + p_dequant++] = table[y];
                new_byte = size;
                y = 0;
            }
        }
